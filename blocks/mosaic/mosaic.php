...
function telabotanica_block_mosaic_item($item)
{
    ...
    echo '<p class="block-mosaic-item-text">' . $item->text . '</p>';
				...
				if (empty($item->button['modifiers']) || !is_array($item->button['modifiers'])) {
				    $item->button['modifiers'] = [];
				}
    if (is_array($item->button['modifiers']) && !in_array('block', $item->button['modifiers'])) {
        $item->button['modifiers'][] = 'block';
    }
    if ($item->i % 2 === 1) {
        $item->button['modifiers'][] = 'orange';
    }
