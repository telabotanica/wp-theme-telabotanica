...
function telabotanica_component_buttons($data)
{
    $defaults = [
    ...
    if ($data->display === 'links' || (is_array($data->display) && in_array('links', $data->display))) {
        $data->modifiers[] = 'as-links';
    }
    if ($data->display === 'seamless' || (is_array($data->display) && in_array('seamless', $data->display))) {
        $data->modifiers[] = 'as-seamless';
    }
    ...
    if (!isset($item->href)) {
        ...
        if (array_key_exists('target', $item->link)) {
            $item->target = $item->link['target'];
        }
        if (array_key_exists('title', $item->link)) {
