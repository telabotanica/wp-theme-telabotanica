...
function telabotanica_component_tools($data)
{
    $defaults = [
    ...
    if (isset($fields->abbr)) {
        $item->description = str_replace($fields->abbr, '<strong>' . $fields->abbr . '</strong>', $item->description);
    }
    ...
    elseif (gettype($item) === 'array') :
          ...
          $item = (object) $item;
    if (!isset($item->icon)) {
        $item->icon = get_template_directory_uri() . '/components/tools/default-icon.svg';
    }
    ...
    echo '<div class="component-tools-item-link"><a href="' . $item->link . '" target="' . $item->link_target . '" style="color: ' . $item->color . '"><span>' . $item->link_text . '</span></a></div>';
    echo '</li>';
