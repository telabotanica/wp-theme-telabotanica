...
function telabotanica_module_list_articles($data)
{
    $defaults = [
		'modifiers' => [],
	];
    ...
    if ($is_event) :
					the_telabotanica_module('event-dates', ['modifiers' => 'float-left']); else :
					printf(
						'<a href="%s" class="list-articles-image">%s</a>',
						esc_url(get_permalink()),
						has_post_thumbnail() ? get_the_post_thumbnail(null, 'post-thumbnail', ['class' => 'list-articles-image']) : ''
					);
    endif;
    ...
    the_telabotanica_module('pagination'); else :
